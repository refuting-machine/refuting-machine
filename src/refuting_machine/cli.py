...
def options_and_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Refuting-Machine thinks for you about the meanings.",
        epilog="Example:  refuting-machine input_text.txt > output_text.txt"
...
def run():
    """
    $ text | refuting-machine                        # Accepts text from the pipe
    $ echo "...<text>..." | refuting-machine         #

    $ refuting-machine multilogue.txt new_turn.txt    # ...or files.
    """
    ...
    try:
        ...
        syslog.openlog(
            ident="refuting-machine",
            logoption=syslog.LOG_NDELAY,
            facility=syslog.LOG_USER
        )
        # Signal (single line less than 4096 only!)
        syslog.syslog(syslog.LOG_INFO, f"refuting-machine: {utterance}.")
        syslog.closelog()
